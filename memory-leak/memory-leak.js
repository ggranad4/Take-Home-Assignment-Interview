(function () {
  ...
  function appendBuffer(segment) {
    ...
    var totalBufferDuration = bufferedSegments.reduce(function (
      duration,
      segment
    ) {
    ...
    console.log(
      "Buffered segment: Segments buffered:",
      buffer.length,
      "Buffer duration (seconds):",
      Math.round(totalBufferDuration),
      "Buffer size (Mb)",
      Math.round(totalBufferSize / 1000000),
      "Memory used (Mb)",
      Math.round(memoryUsed / 1000000)
    );
  }
  ...
  function loadAndParsePlaylist(url, callback) {
    loadPlaylist(url, function (playlist) {
      ...
      for (i = 0; i < lines.length; i++) {
        line = lines[i]; // 00000_mp4 or ##EXTINF:8.008, #EXT-X-ENDLIST
    ...
    loadAndParsePlaylist(playlistUrl, function (segmentList) {
      ...
      function loadAndBufferSegment() {
        ...
        loadSegment(segment.url, function (arrayBuffer) {
          // current segment => {duration, url} and {arrayBuffer}
  ...
  document
    .getElementById("run-test-button")
