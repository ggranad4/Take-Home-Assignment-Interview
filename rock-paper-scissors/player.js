...
class Player {
  constructor(N = null) {
    ...
    if (N) {
      this.knownAddresses = Array.from({ length: N }, (_, i) => 9001 + i);
      if (N % 2 === 1) {
        this.knownAddresses.push("BYE");
      }
    } else {
      this.knownAddresses = [9001, 9002, 9003];
    }
  ...
  onMessage(messageBuffer) {
    ...
    this.logger.log(
      `Received message from ${playerID}: ${method}(${params.join()})`
    );
  ...
  tryToPlay() {
    this.timeout = null;
    const currentopponent = this.sampleKnownAddresses();
    if (currentopponent === "BYE") {
      // dont do anything if bye and just TryToPlay someone else ?? or maybe log hey this player on bye?
      this.tryToPlayRandomDelay();
      return;
    }
  ...
  acceptShot(port, playerID, shot) {
    this.logger.log(
      `Player ${playerID} sent:`,
      shot,
      "; creating counter shot"
    );
    ...
    if (result === "loss") {
      this.logger.log(
        `Beat player ${playerID} with counter shot:`,
        counterShot
      );
      this.wins++;
    } else if (result === "tie") {
      this.logger.log(
        `Tied player ${playerID} with counter shot:`,
        counterShot
      );
      this.ties++;
    } else {
      this.logger.log(
        `Lost to player ${playerID} with counter shot:`,
        counterShot
      );
      this.losses++;
    }
